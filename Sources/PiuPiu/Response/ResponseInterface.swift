...
@MainActor public protocol ResponseInterface: Sendable {
  associatedtype Body
...
public extension ResponseInterface where Body == Data {
  ...
  func decodeAsync<D: Decodable & Sendable>(
    ...
    return try await Task.detached(priority: priority) {
      return try await decoder.decode(type, from: body)
    }.value
  }
}
...
public extension ResponseInterface where Body == Data? {
  ...
  func makeResponseMarkdown() -> String {
    return urlResponse.makeResponseMarkdown(with: urlRequest, body: body)
  }
}
